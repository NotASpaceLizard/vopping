...
**STATUS: DONE — formally executed 2026-09-16, reconfirmed @ pushed sha `8322b59` after the PO's on-device DEFAULT picks, PASS (483/483 combined suite; Part 24 = 29/29 S33 checks), 0 defects. Desktop-verifiable AC fully green. (S33 is the foundation layer. Two registry DEFAULTS were repointed by the PO via the S34 picker on-device — `auto-aisle-run` ⭍ U+2B4D → ↯ U+21AF, `note-toggle` U+1F5CB → ☰ U+2630 — and reconfirmed here; the NB-6 on-device render is PO-confirmed.)**
...
**Locked behavior:** defaults @8322b59 are ⚙ / ✕ / ↯ (auto-aisle-run — PO on-device pick, was ⭍ U+2B4D) / ☰ (note-toggle — PO on-device pick, was astral U+1F5CB) / ✎ / ▲ / ▼ / ✕ / ⌖ / ⭍ (auto-detect-option, unchanged). With no override stored the app renders exactly these (the initial S33/S34 pass proved byte-identity to the pre-picker glyphs; the PO then repointed the two iOS-boxing defaults via the shipped picker). A valid stored override wins, else the registry default; an unregistered id → the U+25A1 fallback (never `undefined`, never a throw). `parseIconOverrides` never throws before render: parse failure / null / non-object / top-level array → `{}`; each value must pass the R17b content constraint or that entry is dropped; a glyph stored for an unregistered id is harmless (never consulted). Static glyphs are set from `iconFor()` at init AND on every override write; `.textContent` is inherently markup-safe (R17a escaping is for the innerHTML draw-sites). `delete-item`/`settings-close` share ✕ and `auto-aisle-run` (default ↯) and `auto-detect-option` (default ⭍) are DISTINCT ids that never cross-set — the PO repointed only the button, not the picker-wheel option.
...
| S33.3b | R-DEFAULT PO picks (no override) | Auto-aisle button ↯ U+21AF + note toggle ☰ U+2630 on every row |
...
| S33.11a-b | R-ISOLATION auto-run/auto-detect | overriding one (non-default ★) never changes the other (distinct defaults: auto-run ↯ vs auto-detect ⭍) |
...
|-----------|----------|--------|-----------|
| S33.1–S33.13 (incl. S33.3b) | as above | as above (verbatim PASS lines in the transcript; e.g. S33.2 offenders=[], S33.3b runGlyph=↯ / noteGlyph=[☰,☰], S33.9 before/after=10/10, S33.12 MIXED del=✗ / name=✎ / up=▲, S33.13 injected=0) | **PASS (29/29)** |

**Overall verdict:** PASS (29/29 S33-specific, reconfirmed @ pushed sha `8322b59`). Full regression re-run clean: `vopping-tests-tester-s1-s27-formal.js` 483/483 (see REGRESSION_LOG.md 2026-09-16 S33+S34 default-retrofit reconfirm row; the 482 initial S33/S34 total all re-confirmed clean against 8322b59 — S33 is a purely additive layer, no mechanism retired). Reproduced x2, zero console/page errors, zero dialogs, zero non-`file://` requests. **PO on-device default picks reconfirmed on desktop (S33.3b, no override):** `#auto-aisle-btn .auto-aisle-glyph` renders ↯ U+21AF and the row note-toggle renders ☰ U+2630 on every row; the panel swatches (S34.2) preview ↯ / ☰ (auto-detect-option still ⭍). **Independent NUL-byte re-verify @8322b59:** a byte-level scan of `script.js`/`index.html`/`style.css` found 0 NUL and 0 stray control bytes (excl. TAB/LF/CR) and `node --check script.js` passes — the Dev's NUL-in-regex fix stays repaired.

## Commands run and output
`node "c:/tmp/pw-test/vopping-tests-tester-s1-s27-formal.js"` — initial-pass transcripts `c:/tmp/pw-test/s33-s34-run1.log` + `s33-s34-run2.log`; @8322b59 default-retrofit reconfirm transcripts `c:/tmp/pw-test/s33-s34-8322b59-run1.log` + `s33-s34-8322b59-run2.log` (Part 24 block, 29/29). Byte scan: `node "c:/tmp/pw-test/vopping-ctrlbyte-scan.js"` → `script.js/index.html/style.css: NUL=0 CR=0 strayControl=0 → CLEAN`.
