...
**STATUS: Tester formal PASS 2026-09-16 (482/482 combined suite; Part 25 = 13/13 S34 checks), 0 defects — DESKTOP-verifiable AC (structure/persistence/routing) fully green. Done-flip pending the PO's parallel on-device NB-6 glyph pick (which candidates RENDER vs. tofu-box on their iPhone) — the shipped picker doubles as that on-device decision tool.**
...
**Overall verdict:** PASS (13/13 S34-specific). Full regression re-run clean: `vopping-tests-tester-s1-s27-formal.js` 482/482 (see REGRESSION_LOG.md 2026-09-16 S33+S34 row). Reproduced x2, zero console/page errors, zero dialogs, zero non-`file://` requests. **Scope note (NB-6):** which candidate glyphs render (tofu vs real) on the PO's iPhone is the real-device acceptance signal, asserted here only via desktop structural proxies — the on-device confirm is the PO's, running in parallel; the Done-flip waits on it. **One test-authoring bug found + fixed while writing this pass (NOT an app defect):** S34.6's scroll check first used `page.click()`, whose auto-scroll to the off-screen candidate moved the panel and made the pre-click scroll reading stale (the app correctly preserved the post-auto-scroll offset, 833) — rewritten to a direct DOM `.click()` so the check isolates the re-render's effect (now 100→100).
